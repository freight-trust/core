...
contract("Management Pseduo Blockchain", function (accounts) {
  ...
  it("attempting to recreate management sidechain id fails", async function () {
    ...
    try {
      ...
      assert.equal(
        err.message,
        "Returned error: VM Exception while processing transaction: revert Blockchain already added -- Reason given: Blockchain already added."
      );
  ...
  it("add an account to the management sidechain id", async function () {
    ...
    await coordInterface.actionVotes(
      common.MANAGEMENT_PSEUDO_BLOCKCHAIN_ID,
      secondParticipant
    );
    ...
    assert.equal(
      isParticipant,
      true,
      "unexpectedly, Second Participant: isBlockchainParticipant == false"
    );
  });
});
