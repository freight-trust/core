...
contract("ERA: Testing a single domain in an ERA", function (accounts) {
  ...
  it("add one domain", async function () {
    ...
    //Note: transactions by default use account 0 in test rpc.
    const resultAddDomain = await eraInterface.addUpdateDomain(
      testDomainHash1,
      testAuthAddress1,
    ...
    const domainOwnerD1 = await eraInterface.getDomainOwner.call(
      testDomainHash1
    );
    ...
    const domainInfoAddr1 = await eraInterface.getDomainInfo.call(
      testDomainHash1
    );
