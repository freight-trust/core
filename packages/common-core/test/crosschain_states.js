...
contract("Crosschain Transaction States:", function (accounts) {
  ...
  const START_MESSAGE_PUBLIC_KEY =
    "";
  //48165743513990300380988978454821709068278076602382423941659282846720703716301
  const A_CROSS_TX_ID =
    "306119615846880469765888303682228643010750279842122396031849905157685439122";
  //102665386630937920234814764809786284335987281145580768364507172654125582397871
  const HASH_OF_START_MESSAGE =
    "78568958157276484761162660378195871061994187815754976194416851188085214982154";
  // Time-out: 138
  const START_MESSAGE_TIMEOUT = "531";
  const KEY_VERSION = "1";
  const START_MESSAGE_SIGNATURE =
    "0x2d4aea78c76d517984086899bb5d5936cdc2e4f598132ff7bb4b5158359dc4ef1ac2546b9726a817adb07087ea9face58d5d493622c3ca3c62ec10b5fc6e6c2e";
  ...
  async function start(coordInterface) {
    ...
    let actionResult = await coordInterface.start(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID,
      HASH_OF_START_MESSAGE,
      START_MESSAGE_TIMEOUT,
      KEY_VERSION,
      START_MESSAGE_SIGNATURE
    );
  }
  ...
  it("notstart", async function () {
    ...
    let state = await coordInterface.getCrosschainTransactionStatus.call(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID
    );
    assert.equal(
      NOT_STARTED,
      state,
      "state for transaction which does not exist should be not started"
    );
  });
  ...
  it("committed", async function () {
    ...
    await coordInterface.commit(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID,
      A_VALID_COMMIT_MESSAGE
    );
    let state = await coordInterface.getCrosschainTransactionStatus.call(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID
  ...
  it("ignored", async function () {
    ...
    await coordInterface.ignore(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID,
      A_VALID_IGNORE_MESSAGE
    );
    let state = await coordInterface.getCrosschainTransactionStatus.call(
      A_BLOCKCHAIN_ID,
      A_CROSS_TX_ID
