...
contract("Add One Blockchain", function (accounts) {
  ...
  it("addBlockchain", async function () {
    ...
    await coordInterface.addBlockchain(
      twoBlockchainId,
      await common.getValidVotingContractAddress(),
  ...
  it("getBlockchainExists for valid blockchain", async function () {
    ...
    const hasD1 = await coordInterface.getBlockchainExists.call(
      twoBlockchainId
    );
  ...
  it("getBlockchainExists for invalid blockchain", async function () {
    ...
    const hasD2 = await coordInterface.getBlockchainExists.call(
      oneBlockchainId
    );
    assert.equal(
      hasD2,
      false,
      "Unexpectedly found blockchain 1, which shouldn't exist"
    );
  });
  ...
  it("getVotingPeriod", async function () {
    ...
    const actualVotingPeriod = await coordInterface.getVotingPeriod.call(
      twoBlockchainId
    );
    assert.equal(
      actualVotingPeriod,
      votingPeriodTen,
      "twoChainId returned unexpected voting period"
    );

    const actualVotingPeriod1 = await coordInterface.getVotingPeriod.call(
      oneBlockchainId
    );
    assert.equal(
      actualVotingPeriod1,
      votingPeriodEleven,
      "oneChainId returned unexpected voting period"
    );
  });
  ...
  it("isBlockchainParticipant", async function () {
    ...
    const isPartBad = await coordInterface.isUnmaskedBlockchainParticipant.call(
      twoBlockchainId,
      accounts[1]
    );
    assert.equal(
      isPartBad,
      false,
      "unexpectedly, account which should not be part of the blockchain is"
    );

    const isPartGood = await coordInterface.isUnmaskedBlockchainParticipant.call(
      twoBlockchainId,
      accounts[0]
    );
    assert.equal(
      isPartGood,
      true,
      "account which should be part of the blockchain is"
    );
  });
  ...
  it("getUnmaskedBlockchainParticipantsSize", async function () {
    ...
    const numUnmasked = await coordInterface.getUnmaskedBlockchainParticipantsSize.call(
      twoBlockchainId
    );
  ...
  it("getMaskedBlockchainParticipantsSize", async function () {
    ...
    const numMasked = await coordInterface.getMaskedBlockchainParticipantsSize.call(
      twoBlockchainId
    );
