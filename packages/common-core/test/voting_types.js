...
contract("Voting: types of voting / things to vote on:", function (accounts) {
  ...
  it("add an unmasked participant", async function () {
    ...
    await coordInterface.proposeVote(
      A_BLOCKCHAIN_ID,
      common.VOTE_ADD_UNMASKED_PARTICIPANT,
      newParticipant,
      "0",
      "0x0"
    );
    ...
    let isParticipant = await coordInterface.isUnmaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      newParticipant
    );
    assert.equal(
      isParticipant,
      true,
      "unexpectedly, New Participant: isUnmaskedBlockchainParticipant == false"
    );

    let numUnmaskedParticipant = await coordInterface.getUnmaskedBlockchainParticipantsSize.call(
      A_BLOCKCHAIN_ID
    );
    assert.equal(
      numUnmaskedParticipant,
      2,
      "unexpectedly, number of unmasked participants != 2"
    );

    let newParticipantStored = await coordInterface.getUnmaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      "1"
    );
    assert.equal(
      newParticipant,
      newParticipantStored,
      "unexpectedly, the stored participant did not match the value supplied."
    );
  });
  ...
  it("add a masked participant", async function () {
    ...
    await coordInterface.proposeVote(
      A_BLOCKCHAIN_ID,
      common.VOTE_ADD_MASKED_PARTICIPANT,
      maskedParticipant,
      "0",
      "0x0"
    );
    ...
    let numMaskedParticipant = await coordInterface.getMaskedBlockchainParticipantsSize.call(
      A_BLOCKCHAIN_ID
    );
    assert.equal(
      numMaskedParticipant,
      1,
      "unexpectedly, number of masked participants != 1"
    );

    let maskedParticipantStored = await coordInterface.getMaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      "0"
    );
    ...
    assert.equal(
      maskedParticipant,
      maskedParticipantStoredHex,
      "unexpectedly, the stored masked participant did not match the value supplied."
    );
  });
  ...
  it("remove an  unmasked participant", async function () {
    ...
    await common.mineBlocks(parseInt(common.VOTING_PERIOD));
    let actionResults = await coordInterface.actionVotes(
      A_BLOCKCHAIN_ID,
      newParticipant
    );
    ...
    await coordInterface.proposeVote(
      A_BLOCKCHAIN_ID,
      common.VOTE_REMOVE_UNMASKED_PARTICIPANT,
      participantToRemove,
      EXPECTED_OFFSET,
      "0x0"
    );
    ...
    await common.mineBlocks(parseInt(common.VOTING_PERIOD));
    let actionResults2 = await coordInterface.actionVotes(
      A_BLOCKCHAIN_ID,
      participantToRemove
    );
    ...
    isParticipant = await coordInterface.isUnmaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      participantToRemove
    );
    assert.equal(
    ...
    assert.equal(
      numUnmaskedParticipant,
      2,
      "unexpectedly, unmasked participants array size != 2"
    );

    newParticipantStored = await coordInterface.getUnmaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      EXPECTED_OFFSET
    );
    assert.equal(
      ...
      newParticipantStored,
      "unexpectedly, the stored participant was not zeroized."
    );
  });
  ...
  it("remove a masked participant", async function () {
    ...
    await common.mineBlocks(parseInt(common.VOTING_PERIOD));
    let actionResult1 = await coordInterface.actionVotes(
      A_BLOCKCHAIN_ID,
      maskedParticipant
    );
    ...
    await coordInterface.proposeVote(
      A_BLOCKCHAIN_ID,
      common.VOTE_REMOVE_MASKED_PARTICIPANT,
      maskedParticipant,
      EXPECTED_OFFSET,
      "0x0"
    );
    await common.mineBlocks(parseInt(common.VOTING_PERIOD));
    let actionResult2 = await coordInterface.actionVotes(
      A_BLOCKCHAIN_ID,
      maskedParticipant
    );
    ...
    maskedParticipantStored = await coordInterface.getMaskedBlockchainParticipant.call(
      A_BLOCKCHAIN_ID,
      EXPECTED_OFFSET
    );
    ...
    assert.equal(
      0,
      maskedParticipantStoredHex,
      "unexpectedly, the stored masked participant was not zeroized."
    );
  });
