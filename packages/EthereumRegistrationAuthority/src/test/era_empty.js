...
contract("ERA: Empty Tests", function (accounts) {
  ...
  it("getDomainOwner", async function () {
    let eraInterface = await common.getDeployedERA();
    const domainOwner1 = await eraInterface.getDomainOwner.call(
      testDomainHash1
    );
    assert.equal(
      domainOwner1,
      0,
      "Unexpectedly, call getDomainOwner on a domain that doesn't exist didn't return 0"
    );
  });
  ...
  it("getAuthority", async function () {
    ...
    assert.equal(
      authority,
      0,
      "Unexpectedly, call getAuthority on a domain that doesn't exist didn't return 0"
    );
  });
  ...
  it("getDomainInfo", async function () {
    ...
    assert.equal(
      orgInf,
      0,
      "Unexpectedly, call getDomainInfo on a domain that doesn't exist didn't return 0"
    );
  });
});
