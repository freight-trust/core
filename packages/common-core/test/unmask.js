...
contract("Unmasking masked participants:", function (accounts) {
  ...
  it("unmask", async function () {
    ...
    let salt = "0x" + salt1;
    let maskedParticipant = await addMaskedParticipant(
      coordInterface,
      newParticipant,
      salt1
    );
    ...
    assert.equal(
      isParticipant,
      false,
      "unexpectedly, New masked participant: isUnmaskedBlockchainParticipant != false"
    );
    ...
    await coordInterface.unmask(A_BLOCKCHAIN_ID, EXPECTED_OFFSET, salt, {
      from: newParticipant,
    });
    ...
    assert.equal(
      isParticipant,
      true,
      "unexpectedly, unmasked participant doesnt exist: isUnmaskedBlockchainParticipant == false"
    );
    ...
    assert.equal(
      "0x0",
      maskedParticipantStoredHex,
      "unexpectedly, the stored masked participant not zeroized."
    );
  });
